...
class FingerChannel extends ChannelMethod {
  ...
  Future<String>captureFingerISODP() async{
    String resul = "";
    try {
      Map<Object?,Object?> res= await methodChannel.invokeMethod(captureFingerDP);
  ...
  Future<String> captureNameDeviceDP() async {
    ...
    try {
      ...
      Map<Object?,Object?> res= await methodChannel.invokeMethod(captureNameDevice);
      final state=res["state"].toString();UtilMethod.imprimir ("res[state]:+$state");
      vNameDevice=res["state"].toString()=="01"?"":res["message"].toString();UtilMethod.imprimir ("res[message]:"+res["message"].toString());
    } catch (e) {
      UtilMethod.imprimir ("excepcion:+$e");
      vNameDevice = "";
    }
  ...
  final String huellaDP="";
}
